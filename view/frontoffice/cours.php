...
if (isset($_GET['fav'])) {
	...
	$f = $_GET['f'];
	$coursC = new coursC();
	$coursC->createfav($u, $f);
} else if (isset($_GET['search'])) {
	$coursC = new coursC();
	$cours = $coursC->search($_GET['search']);
} else {
	$coursC = new coursC();
	$cours = $coursC->read();
}
				...
				<div class="col-md-12">
					<div class="product-filters">
						<ul>
							...
							<li data-filter=".svt">svt</li>
							<li data-filter=".histoire">histoire</li>
						</ul>
					</div>
				</div>
			</div>
				...
				?>
					<div class="col-lg-4 col-md-6 text-center <?=
						...
						<div class="single-product-item" style="background-color: grey;">
							...
							<p class="product-price"><span><?= $f['cour'] ?></span></p>
							<a href="?fav&&f=<?= $f['id'] ?>&&u=<?= $_SESSION['id'] ?>" class="cart-btn"><i class="fas fa-plus"></i>Ajouter favories</a>
						</div>
					</div>
				<?php
				}
